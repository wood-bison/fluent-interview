...
1. `/`, `/program?track=node`, `/atlas`, `/questions` — маршрут, граф и каталог;
2. `/practice`, `/practice/node-event-loop-001`,
   `/practice/lesson/node-event-loop`, `/review` — полный учебный цикл;
...
## G12-025 — Today route state-evidence wave (2 сентября 2026)

Target-коммит без push — `fc2f24f26738889ccee795a003b52aa9139b0a3f`.

В рамках corrective evidence-среза live Today route был проверен на свежем
compose-project-scoped стеке и зафиксирован не одним снимком, а пятью
переиспользуемыми состояниями learner shell: `ready`, `in-progress`,
`selected-station`, `scroll-end` и `rail-collapsed`. Для каждого состояния
сохранены interaction trace, semantic snapshot и реальный visual JPEG. В
семантике явно проверены `lang=en`, `data-theme=dark`, sole main scroll owner
`.app-scroll-region`, достижение `scrollTop=scrollMax` в конце страницы и
переключение навигационного rail `expanded → collapsed`.
...
- migrations **18/18**, pending **0**;
- Today route: `clientHeight=660`, `scrollHeight=2130`,
  `overflow-y=auto`, `scrollMax=1470`, `atEnd=true`;
- state registry generated by `pnpm design:state-evidence`: status `OPEN`,
  `G12-025`, `screens=12`, `states=71`, `evidenceReadyStates=5`,
  `openStates=66`, `openDispositions=12`, `unresolvedItems=78`,
  `structuralFailureCount=0`;
- `sha256sum -c docs/verification/greenfield/G12/checksums.sha256` — all
  entries PASS;
- focused state-evidence registry tests **4/4 PASS**;
- exact target ladder `git status → git diff --check → NX_CI=1 pnpm check →
...
- `fluent-interview-platform/docs/verification/greenfield/G12/state-evidence/today-route/`;
- `fluent-interview-platform/docs/verification/greenfield/G12/state-evidence/today-route-wave-2026-09-02.md`;
...
Защитный тест исторического G10S evidence-индекса теперь читает immutable
anchor через `git show`, поэтому новый G12 evidence может расширять текущий
индекс (с **428** до **446** записей), не переписывая handoff-доказательство
старого gate. Focused G10S 242/244 и schema tests — **9/9 PASS**.

Счётчик master-плана не изменился: **658 / 476 / 1134 / 58,02%**. Это
evidence-only срез: новые учебные вопросы, задачи, activity и progress records
не добавлялись. Сам G12-025 остаётся открытым до полного покрытия registry и
human disposition всех состояний; текущая волна закрывает 5 из 71 состояний,
оставляя **66** состояний, **12** disposition и **78** unresolved items.
Следующая очередь: Program/Atlas state evidence, G10S-246 human acceptance,
...
evidence`). Волна продолжает live-проверку после Practice workbench и закрывает
все пять критических состояний маршрута `/practice/lesson/[id]`: выбор
вопроса, опубликованное задание, честный preview, fail-closed неизвестного
вопроса и русскую локаль.

Покрыты состояния:

- `question-selected` — второй опубликованный Node.js вопрос имеет только
  observe-activity, поэтому ложная runnable-ссылка не появляется;
- `activity-released` — канонический вопрос показывает две assessed
  activities и явный переход в workbench;
- `activity-preview` — плановый `js-scheduling` без released card не
  маскируется под готовую задачу и предлагает возврат в Questions/Program;
- `missing-question` — неизвестный `question` query отклоняется без
  молчаливой подмены, сохраняется lesson/track context;
- `ru` — `html[lang]=ru`, русская копия и те же Node.js placements/activity
  identity остаются согласованными.
...
их SHA-256 добавлены в `G12/state-evidence/index.json` и
`G12/checksums.sha256`. Канонический отчёт:
`fluent-interview-platform/docs/verification/greenfield/G12/state-evidence/lesson-wave-2026-09-02.md`.
...
- live URL `http://127.0.0.1:47360/`, session
  `7afeba77-21d6-4238-8aa8-1d822d99e574`, **6/6** services healthy;
- migrations **18/18**, pending **0**;
- единственный владелец вертикальной прокрутки `.app-scroll-region` во всех
  пяти маршрутах;
- state registry: `stateHash=d5133cdb…`, `screens=12`, `states=71`,
  `evidenceReadyStates=42`, `openStates=29`, `openDispositions=12`,
  `unresolvedItems=41`, `structuralFailureCount=0`;
- G10S evidence index пересобран до **563** historical metadata-only записей;
- checksum manifest и focused state-registry tests **4/4 PASS**;
- полный target ladder (`git status → git diff --check → NX_CI=1 pnpm check →
  pnpm boundary:check → pnpm toolchain:check → git commit`) — PASS.
...
Счётчик master-плана не изменился: **658 / 476 / 1134 · 58,02%**. Это
evidence-only срез без новых curriculum questions, tasks или progress records.
G12-025 остаётся `OPEN` до покрытия следующих **29** состояний и **12** human
dispositions. Следующая bounded очередь: Review, Projects, Studio,
...
После row-level readiness hardening target получил четыре локальных commits:
...
`b1a5fc4` (`test(readiness): guard full working tree writes`) и `289f1e3`
(`test(readiness): report per-command write violations`). Push не
...
а также неизменность `git status --porcelain` после каждой команды. Явный write-path
...
toolchain и `git diff --check` — **PASS**; чистые target `b1a5fc4` и
`289f1e3` подтвердили `target.clean=true`. Это только hardening воспроизводимости,
